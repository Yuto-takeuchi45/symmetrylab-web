...
- [ ] Google広告アカウントで自動タグ設定を有効化
- [ ] GA4プロパティとウェブデータストリームを作成し、測定ID（`G-...`）を確認
- [ ] Google広告で「無料相談申込」のコンバージョンアクションを作成し、コンバージョンID（`AW-...`）とラベルを確認
- [ ] GTMコンテナを作成し、`SYMMETRY_GTM_CONTAINER_ID`（`GTM-...`）をRenderへ設定
- [ ] GTMで`generate_lead`カスタムイベントトリガーを作成
- [ ] GTMでGA4イベントタグを作成し、イベント名を`generate_lead`に設定
- [ ] GTMでGoogle広告コンバージョンタグを作成し、ID・ラベル・`transaction_id`を設定
- [ ] Google広告のコンバージョンカウントを「1回」に設定
- [ ] GA4インポートとGoogle広告タグを同時に主要コンバージョンへ設定しない
- [ ] GTMコンテナを公開し、GTMプレビューで本番ドメインを検証
- [ ] Renderの`/api/tracking-config`で本番GTM IDが返ることを確認

### 11-3. 本番申込の必須テスト

- [ ] Google広告のテスト用URLまたはUTM付きURLでLPへ流入し、GCLID・UTMが保存されることを確認
- [ ] 正常申込でDBに1件だけ保存され、管理画面に表示されることを確認
- [ ] 正常申込で`generate_lead`が1回だけ発火することを確認
- [ ] 入力エラー・通信エラーでは`generate_lead`が発火しないことを確認
- [ ] 同じ申込を再送しても、DB・メール・コンバージョンが重複しないことを確認
- [ ] GA4 DebugViewに氏名、メール、電話番号などの個人情報が出ないことを確認
- [ ] 満席・受付対象外日時・ブロック日が選択できないことを確認
- [ ] PCとスマートフォンでフォーム送信、完了表示、メール受信まで確認
- [ ] プライバシーポリシーの内容と申込フォームの同意文面を確定

### 11-4. Google広告キャンペーンの必須設定

- [ ] 広告の最終URLを本番LP（`/consulting-career`）に設定
- [ ] 配信地域、言語、予算、入札戦略、広告スケジュールを確定
- [ ] キーワード、マッチタイプ、除外キーワードを登録
- [ ] 広告文・表示URL・リンク先が実際のLP内容と一致することを確認
- [ ] Google広告の審査・ポリシー違反・請求設定に問題がないことを確認
- [ ] 初回配信後、検索語句レポート、コンバージョン、メール通知、管理画面を毎日確認する担当者を決める

### 11-5. このリポジトリで設定する環境変数

| 変数 | 必須条件 | 設定場所 |
| --- | --- | --- |
| `BASE_URL` | 本番URLを設定 | Render |
| `ADMIN_KEY` | 強い秘密値を設定 | Render |
| `ADMIN_EMAIL` | 管理者通知先を設定 | Render |
| `RESEND_API_KEY` | Resend利用時に設定 | Render |
| `RESEND_FROM_EMAIL` | Resend認証済み送信元を設定 | Render |
| `SYMMETRY_GTM_CONTAINER_ID` | GTM利用時に設定 | Render |
| `SYMMETRY_GA4_MEASUREMENT_ID` | GTMを使わない直接タグ構成時のみ設定 | Render |
| `SYMMETRY_GOOGLE_ADS_CONVERSION_ID` | GTMを使わない直接タグ構成時のみ設定 | Render |
| `SYMMETRY_GOOGLE_ADS_CONVERSION_LABEL` | GTMを使わない直接タグ構成時のみ設定 | Render |
| `DB_PATH` | `/var/data/bookings.db`から変更しない | Render設定済み |

推奨構成はGTMを使う構成です。この場合、GA4測定IDとGoogle広告のコンバージョンID・ラベルはGTM内に設定し、LP側の直接タグ用環境変数は空欄のままにします。GTMが未設定のときだけ、直接タグ用環境変数が使われます。
